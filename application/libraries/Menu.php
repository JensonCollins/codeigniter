...
class Menu
{
    public function dynamicMenu()
    {
        ...
        if ($user_type != 1) {
            ...
            $query = "SELECT tbl_user_role.menu_id, tbl_user_role.employee_login_id, tbl_menu.*
                FROM tbl_user_role
                INNER JOIN tbl_menu
                ON tbl_user_role.menu_id=tbl_menu.menu_id
                WHERE tbl_user_role.employee_login_id=$employee_login_id
                ORDER BY sort;";
            $user_menu = $CI->db->query($query)->result_array();
        } else { // get all menu for admin
            $query = 'SELECT menu_id, label, link, icon, parent FROM tbl_menu ORDER BY sort';
            $user_menu = $CI->db->query($query)->result_array();
        }
