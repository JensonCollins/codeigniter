        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
				...
				<form role="form" enctype="multipart/form-data" id="addOrdersForm"
                      action="<?php echo base_url(); ?>admin/jobs/save_job/<?php if (!empty($orders->order_id)) {
				...
				<div class="col-md-12 col-sm-12">
				    ...
				    <div class = "form-group col-md-6 col-sm-6">
						<label for="customer_id">Add Customer</label>
						<div class="form-control" style="border:0px;">
							<a href="<?php echo base_url('admin/customer/add_customer') ?>" target = "_blank" class="btn btn-customs btn-xs" title="" data-toggle="tooltip" data-placement="top" data-original-title="Add Customer">Click Here</a>
						</div>
				    </div>
				</div>  
				...
				<?php 
					...
					<input type="hidden" name="order_part_id" value="<?php echo $order_parts->id ?>">
					<div class="col-md-12 col-sm-12">
					    <div class = "form-group col-md-6 col-sm-6">
							<label for="sel_parts">Parts</label><a href="<?php echo base_url('admin/parts/add_parts') ?>" target = "_blank" class="btn btn-customs btn-xs" title="" data-toggle="tooltip" data-placement="top" data-original-title="Add Part">Click Here to Add New Part</a>
							<select class="form-control select2" id="sel_parts_<?php echo $order_parts->part_id;?>" name="sel_parts" style="width: 100%;">
							<option value="">Select Parts</option>
							<?php 
							foreach ($parts as $pc_qt_value) 
							{   //print_r($pc_qt_value);
							if($order_parts->part_id == $pc_qt_value->id)
							{
					    ...
					    <div class = "form-group col-md-3 col-sm-3">
					     <label for="part_qty">Status: <span class="required">*</span></label>  
							<select class="form-control input-sm" name="mode_payment" id="mode_payment">
								...
								<option value="Pending" <?php if($order_parts->Status == 'Pending') { echo 'selected'; } else { echo ''; } ?>>Pending</option>
								<option value="On Hold" <?php if($order_parts->Status == 'On Hold') { echo 'selected'; } else { echo ''; } ?>>On Hold</option>
								<option value="Invoiced" <?php if($order_parts->Status == 'Invoiced') { echo 'selected'; } else { echo ''; } ?>>Invoiced</option>
								<option value="Complete" <?php if($order_parts->Status == 'Complete') { echo 'selected'; } else { echo ''; } ?>>Complete</option>
								<option value="Cancelled" <?php if($order_parts->Status == 'Cancelled') { echo 'selected'; } else { echo ''; } ?>>Cancelled</option>
							</select>
					    </div>
					</div> 
					...
					<div class="col-md-12 col-sm-12">
						<div class = "form-group col-md-3 col-sm-3">
					     <label for="part_qty">Job Notes:<span class="required">*</span></label>   
							...
							if (!empty($order_parts->Notes)) {
							    echo $order_parts->Notes;
							}
							?></textarea>
							<?php //echo display_ckeditor($editor['ckeditor']); ?>
					    </div>
						...
						if (!empty($order_parts->OrderPrice)) {
						echo $order_parts->OrderPrice;
						}
						?>"
							class="form-control">
					    </div>
